...
function RichTextEditor({ value, onChange }) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "min-h-[180px] w-full rounded-2xl border border-slate-200 bg-white p-4 prose-rich outline-none focus:ring-2 focus:ring-primary-200",
      },
    },
  });
  ...
  useEffect(() => {
    if (editor && value !== editor.getHTML()) {
      editor.commands.setContent(value || "<p>Start writing…</p>", {
        emitUpdate: false,
      });
    }
  }, [editor, value]);
...
function PortfolioPage() {
  ...
  // Robust ownership check (checks user ID against portfolio owner ID or fallback username match)
  ...
  // Reactive edit mode sync with query params and ownership
  useEffect(() => {
    const isEditRequested = searchParams.get("edit") === "true";
    setEditing(isEditRequested && isOwner);
  ...
  if (error || !portfolio) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-24 text-center">
        <h1 className="text-4xl font-black">Portfolio not found</h1>
        {error && (
  ...
  const designStyles = {
    background:
      ...
      portfolio.design_settings?.background ||
      "#f8fafc",
    color:
      pendingDesign?.text_color ||
      portfolio.design_settings?.text_color ||
      "#0f172a",
    fontFamily:
      pendingDesign?.font_family ||
      portfolio.design_settings?.font_family ||
      "Inter, sans-serif",
    fontSize: `${
      pendingDesign?.base_font_size ||
      portfolio.design_settings?.base_font_size ||
      18
    }px`,
    "--heading-scale":
      pendingDesign?.heading_scale ||
      portfolio.design_settings?.heading_scale ||
      1,
  };

  return (
    <div className="pt-8" style={designStyles}>
      {/* Floating Toolbar for Portfolio Owners */}
      {isOwner && (
        <div className="sticky top-4 z-40 mx-auto mb-6 flex max-w-5xl items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white/90 p-3 shadow-lg backdrop-blur-xl">
          <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
            <Sparkles className="h-4 w-4 text-primary-600" />
            {editing
              ? `Editing Mode – ${username}`
              : `Viewing Public Portfolio`}
          </div>
          ...
          <div className="flex gap-2">
            {editing ? (
              <>
                ...
                <Button variant="muted" onClick={() => setShowAddSection(true)}>
                  Add Section
                </Button>
                <Button variant="muted" onClick={() => setSearchParams({})}>
                  Cancel
                </Button>
              </>
            ) : (
        ...
        {visibleSections.map((section) => {
          ...
          if (key === "hero") {
            return (
              <section
                key={key}
                className="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm md:p-12"
              >
                  ...
                  <div>
                    ...
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html:
                          portfolio.bio ||
                          "<p>Creative developer building meaningful digital experiences.</p>",
                      }}
                    />
                    {isOwner && editing && (
                      <button
            ...
            return (
              <section key={key} className="py-16">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: portfolio.bio || "<p>More about this creator…</p>",
                  }}
                />
              </section>
            );
            ...
            return (
              <section key={key} className="py-10">
                ...
                <div className="flex flex-wrap gap-3">
                  {(portfolio.skills || "FastAPI, React, Design, Strategy")
                    .split(",")
                    .map((skill) => (
                      <span
                        key={skill}
                        className="inline-flex items-center gap-2 rounded-full border border-primary-200 bg-primary-50 px-4 py-2 text-sm font-semibold text-primary-700"
                      >
              ...
              <section key={key} id="contact" className="py-12">
                <div className="rounded-[2rem] bg-slate-950 p-8 text-white">
                  ...
                  <div className="mt-6 flex flex-wrap gap-4 text-sm text-slate-300">
                    <a
                      href={`mailto:${portfolio.contact?.email || portfolio.user?.email}`}
                      ...
                      Object.entries(portfolio.social_links).map(
                        ([label, href]) => (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-2 rounded-full border border-slate-700 px-4 py-2 hover:border-slate-500"
                          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4">
          <div className="w-full max-w-2xl rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl">
            ...
            <div className="mt-4 flex justify-end gap-3">
              <Button variant="muted" onClick={() => setEditorState(null)}>
                Cancel
              </Button>
      ...
      {isOwner && editing && showDesignPanel && (
        <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md border-l border-slate-200 bg-white p-6 shadow-2xl">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                min="14"
                max="22"
                value={pendingDesign?.base_font_size || 18}
                onChange={(event) =>
                  setPendingDesign((prev) => ({
                    ...prev,
                    base_font_size: Number(event.target.value),
                  }))
                }
                className="w-full"
              />
            ...
            <div>
              ...
              <input
                ...
                max="1.5"
                step="0.05"
                value={pendingDesign?.heading_scale || 1}
                onChange={(event) =>
                  setPendingDesign((prev) => ({
                    ...prev,
                    heading_scale: Number(event.target.value),
                  }))
                }
                className="w-full"
              />
            ...
            <div>
              ...
              <select
                value={pendingDesign?.font_family || "Inter, sans-serif"}
                onChange={(event) =>
                  setPendingDesign((prev) => ({
                    ...prev,
          ...
          <div className="mt-8 flex justify-end gap-3">
            <Button variant="muted" onClick={() => setShowDesignPanel(false)}>
              Close
            </Button>
            <Button
              onClick={() => {
                setPortfolio((prev) => ({
                  ...prev,
                  design_settings: pendingDesign || prev.design_settings,
                }));
          ...
          <div className="w-full max-w-md rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl">
            ...
            <div className="space-y-3">
              {[
                ...
                const exists = (portfolio.sections || []).some(
                  (section) => section.section_type === name && section.visible,
                );
                return (
                  <button
                    key={name}
